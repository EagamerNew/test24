    ...
    this.commonService.getExamHistoryByUserId(this.docUserId).then(res => {
      this.examHistory = res.map(mapper => {
        if (mapper.date) {
          ...
          const custDate = mapper.date.split('-');
          const custHour = mapper.time.split(':');
          const finalDate = new Date();
          finalDate.setFullYear(custDate[2], custDate[1], custDate[0]);
          finalDate.setHours(custHour[0], custHour[1]);
          mapper['finalDate'] = finalDate;
        }
        return mapper;
      });
    })
